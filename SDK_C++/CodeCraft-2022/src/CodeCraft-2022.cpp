...
int cost_max_index[MAXT], can_full_use_time[MAXT];
...
struct Producer {
    ...
    int GetScore() {
        ...
        // }        
        // ?
        int ret = p[cost_max_index[cost_index]];
        delete[] p;
        return ret;
    }
    double GetAnswer(int update_has_cost, int update_full_use_time) {
        ...
        sort(p + 1, p + 1 + times); 
        double ret = CalCost(bandwidth, p[cost_max_index[cost_index]]);
        if (p[times] == 0) ret = 0.0;
        // 更新容量
        if (update_has_cost) {
            has_cost = max(base_cost, p[cost_max_index[cost_index]]);
        }
        if (update_full_use_time) {
            ...
            int num = 0;
            for (int i = 1; i <= times; ++i) {
                if (num == can_full_use_time[cost_index]) break;
                if (time_cost[i] <= p[cost_max_index[cost_index]]) continue;
                ++num;
            ...
            for (int i = 1; i <= times; ++i) {
                int time = indexs[i];
                if (num == can_full_use_time[cost_index]) break;
                if (time_cost[time] < p[cost_max_index[cost_index]]) continue;
                ++num;
...
void Init() {
    ...
    for (int i = 1; i <= 99; ++i) {
        ::cost_max_index[i] = (times * i + 99) / 100;
        ::can_full_use_time[i] = times - cost_max_index[i];
    }
        ...
        for (int time = 1; time <= times; ++time) {
            for (int stream_id = 1; stream_id <= pt[time]; ++stream_id) {
                ...
                if (consumers[consumer_id].time_need[time][stream_id] > 0) {
                    ...
                    time_consumer_vec[time].emplace_back(PP(P(consumers[consumer_id].time_need[time][stream_id] / consumer_nd_block, -(int)consumers[consumer_id].can_visit_point_vec.size()), 
                                                            P(consumer_id, stream_id)));
                }                    
    ...
    for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
        producers[producer_id].cost_index = 95;
    }
    ...
    for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
        ...
        int v = tmp_vec[cost_max_index[90] - 1];
    ...
    for (int i = 0; i < 10; ++i) producers[producer_vec[i].second].cost_index = 90;

    if (debug_file != nullptr) {
        for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
            (*debug_file) << "init_cost_index, " 
                        << " producer_id: " << producer_id
                        << " cost_index: " << producers[producer_id].cost_index
                        << endl;
        }
    }
...
void CheckVaild() {
    if (result_file != nullptr && info[0] != '!') {
        ...
        for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
            for (int time = 1; time <= times; ++time) {
                for (auto &info : producers[producer_id].consumer_set[time]) {
                    int consumer_id = info / BASE;
                    int stream_id = info % BASE;
        ...
        if (check_flag == 0) {
            (*result_file) << "check fail! in_producer" << endl;
        }
        check_flag = 1;
        for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
            for (int time = 1; time <= times; ++time) {
                if (producers[producer_id].time_cost[time] != tmp_sum[producer_id][time]) {
                    check_flag = 0;
                }
            }
        }
        if (check_flag == 0) {
            (*result_file) << "check fail! cost_sum" << endl;
        }    
        check_flag = 1;
        for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
            for (int time = 1; time <= times; ++time) {
                if (producers[producer_id].discard && tmp_sum[producer_id][time] > 0) {
                    check_flag = 0;
                }
            }
        }
        ...
        check_flag = 1;
        for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
            if (producers[producer_id].has_use_full_time != can_full_use_time[producers[producer_id].cost_index]) {
                check_flag = 0;
            } 
        }
        if (check_flag == 0) {
            (*result_file) << "check fail! full_use" << endl;
        }    
    } 
}
...
void PreWork(int nd_write, int left_weight, int right_weight) {
    ...
    int upk = 0;
    for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
        upk += can_full_use_time[producers[producer_id].cost_index];
    }
    for (int k = 1; k <= upk; ++k) {
        ...
        for (int producer_id = 1; producer_id <= producer_number; ++producer_id) {
            if (producers[producer_id].has_use_full_time == can_full_use_time[producers[producer_id].cost_index]) continue;
...
void PreFirst(int nd_write) {
    ...
    for (auto& p : tmp) {
        ...
        for (int i = 0; i < can_full_use_time[producers[producer_id].cost_index]; ++i) {
...
void PreWork(int nd_write) {
    ...
    if (debug_file != nullptr && is_ab == 0) {
        ...
        for (int i = 1; i <= min(producer_number, 10); ++i) {
            ...
            for (int time = cost_max_index[producers[i].cost_index] ; time <= times; ++time) {
    ...
    for (int i = 0; i < producer_vec.size(); ++i) {
        ...
        for (auto& tp : time_vec) {
            ...
            for (auto& cp : producers[from_producer_id].consumer_set[time]) {
                ...
                if (loop_type == 0) down = 0;
                if (loop_type == 1) down = i;
                for (int j = (int)producer_vec.size() - 1; j >= down; --j) {
                    if (j == i) continue;
                    to_producer_id = producer_vec[j].second;
                    if (producers[to_producer_id].can_visit_point[consumer_id] == 0) continue;
                    if (producers[to_producer_id].is_full_use_time[time] == 1) {
...
int tmp_up[MAXT][MAXN];
int tmp_belong[MAXT][MAXN][MAXK];
void EndTryDiscard() {
    clock_t func_begin_time = clock();
    // todo
    vector<P> time_vec;
    ...
    double min_cost;
    double up_cost;
    for (int i = 0; i < producer_vec.size() / 3; ++i) {
        from_producer_id = producer_vec[i].second;
        time_vec.clear();
        tmp.clear();
        can_discard = 1;
        up_cost = 0.0;
        if (debug_file != nullptr) {
            (*debug_file) << "discard_begin:"
                          << " producer_id: " << from_producer_id
                          << endl;
        }
        for (int time = 1; time <= times; ++time) {
            // if (producers[from_producer_id].is_full_use_time[time] == 1) continue;
        ...
        for (auto& tp : time_vec) {
            ...
            for (auto& cp : producers[from_producer_id].consumer_set[time]) {
                ...
                min_cost = 1e12;
                best_to_producer_id = -1;
                for (int j = (int)producer_vec.size() - 1; j >= 0; --j) {
                    if (i == j) continue;
                    to_producer_id = producer_vec[j].second;
                    if (producers[to_producer_id].discard) continue;
                    if (producers[to_producer_id].can_visit_point[consumer_id] == 0) continue;
                    if (producers[to_producer_id].TimeRemain(time) < nd_bandwidth + tmp_up[time][to_producer_id]) continue;
                    double tmp = CalCost(producers[to_producer_id].bandwidth, nxt_hascost[to_producer_id], producers[to_producer_id].time_cost[time] + tmp_up[time][to_producer_id], nd_bandwidth);
                ...
                if ((up_cost + min_cost) * 1.1 > (double)base_cost) {
                    can_discard = 0;
                    break;
                }
                up_cost += min_cost;
                tmp_belong[time][consumer_id][stream_id] = best_to_producer_id;
                tmp_up[time][best_to_producer_id] += nd_bandwidth;
                if (producers[best_to_producer_id].is_full_use_time[time] == 0) {
                    nxt_hascost[best_to_producer_id] = max(nxt_hascost[best_to_producer_id], producers[best_to_producer_id].time_cost[time] + tmp_up[time][best_to_producer_id]);
                }
            }
        }
        // 其实可以break了
        if (can_discard == 0) {
            ...
            producers[from_producer_id].discard = 1;
            for (int time = 1; time <= times; ++time) {
                for (auto& cp : producers[from_producer_id].consumer_set[time]) {
                    consumer_id = cp / BASE;
                    stream_id = cp % BASE;
                    nd_bandwidth = consumers[consumer_id].ini_time_need[time][stream_id];
                    MoveSomeBandWidth(time, from_producer_id, tmp_belong[time][consumer_id][stream_id], consumer_id, stream_id, nd_bandwidth, 1, 0, 1);
                }
                producers[from_producer_id].consumer_set[time].clear();
            }
            for (int j = 1; j <= producer_number; ++j) producers[j].GetAnswer(1, 1);
            if(debug_file != nullptr) {
                (*debug_file) << "discard," 
                                << " producer_id: " << from_producer_id
                                << " up_cost: " << up_cost 
                                << " base_cost: " << base_cost
                                << " degree: " << producers[from_producer_id].can_visit_point_vec.size()
                                << endl;
            }
        }
    }
...
void sig_handler(int num) {
    ...
    if (result_file != nullptr && debug_file != nullptr && is_ab == 0) {
        ...
        for (int i = 1; i <= producer_number; ++i) {
            ...
            if (times <= 100) {
                for (int time = 2; time <= cost_max_index[producers[i].cost_index]; ++time) {
                ...
                for (int time = 2; time <= cost_max_index[producers[i].cost_index]; time += times * 2 / 100 ) {
                    (*debug_file) << " " << producers[i].time_cost[time];
                    if (num == 20) {
                        num = 0;
                        (*debug_file) << endl;
            ...
            (*debug_file) << "last5%: ";
            for (int time = cost_max_index[producers[i].cost_index] + 1; time <= times; ++time) {
                (*debug_file) << " " << producers[i].time_cost[time];
                if (num == 20) {
                    num = 0;
                    (*debug_file) << endl;
